...
**目標**: 提供 CLI 和 UI 介面，用於管理 Claude Code 的專案和會話資料
...
| `todos/{session_id}-agent-{session_id}.json` | 待辦事項 JSON | 精確匹配 session ID 刪除 |
| `plans/*.md` | 已儲存的 plans（Markdown，位於 `~/.claude/plans/`） | 刪除 session 時：若該 plan 只被該 session 引用則刪除；若為多 session 共用則保留 |
| `sessions/{id}.json` | Session 元數據 | 直接刪除 |
| `sessions/*.json` | Session marker files（依 PID 產生） | 刪除 session 時，按內容/命名規則匹配並刪除相關 marker |
| `teams/{session_id}/` | Team data directory | 直接刪除整個目錄 |
| `session-env/{session_id}/` | Session 環境變數 | 直接刪除目錄 |
| `history.jsonl` | 對話歷史，每行含 sessionId | 需流式重寫移除特定 session |
...
| `file-history/{session_id}/` | 檔案編輯歷史快照 (內含 `{hash}@v{N}` 版本) | 按目錄名 session_id 直接匹配 |
| `transcripts/{path_hash}/` | 對話記錄，按工作目錄組織 | 需掃描識別 session |
| `shell-snapshots/snapshot-*.sh` | Shell 環境快照 | 無明確 session 關聯，按時間推斷 |

**paste-cache 共用情況** (已驗證):

```
4bdb56ddb49c6c5f.txt -> 2 sessions (0a339257..., 78d43d84...)
543be4bddf5d1057.txt -> 2 sessions (0a339257..., 78d43d84...)
其餘 -> 1 session each
```

刪除規則: 當刪除某 session 時，若 paste-cache 檔案僅被該 session 引用，則刪除；若被其他 session 引用，則保留。需掃描 history.jsonl 建立引用計數。

plans 刪除規則: plans 為 `~/.claude/plans/*.md`。當刪除某 session 時，若 plan 只被該 session 引用則刪除；若多 session 共用則保留。
...
- **Session → 專案**: 透過 `workingDirectory` 關聯 (存於 transcripts、history)
...
│   ├── 012e2e2d-fde5-4146-bc11-15a436a8d46b (2024-03-05, in_progress)
│   ├── 2acfdebe-53d1-47de-8563-4614936dada6 (2024-03-05, completed)
│   └── 51bd6114-6234-48b1-b03b-598b192eeb78 (2024-03-05, completed)
├── ~/Documents/Dev/Real-Alpha-Trader (1 session)
│   └── 67a0789d-d082-4b67-8c74-22dc52cf696c (2024-03-04, completed)
└── (No project) (2 sessions)
    ├── 8007497a-c66c-4ff1-8ca2-e4209bf27e26
    └── 92b8b607-cbc3-4789-8ec6-797c447f2428

Global sessions without project: 2
Total sessions: 8
```
...
- `--verbose` - 顯示更多資訊 (如 task 數量、plan 數量)
...
4. `~/.claude/sessions/*.json` - Session 元數據與 PID marker files（依內容/命名匹配 session 關聯後刪除）
...
10. `~/.claude/plans/*.md` - plans 為 Markdown；若 plan 只被該 session 引用則刪除，若共用則保留
11. `~/.claude/transcripts/` - 需掃描內容識別 session
...
1. **所有關聯會話**:
   - 該專案的所有 session 資料 (依 3.2 邏輯刪除)

2. **專案本地數據** (可選):
   - 僅在指定 `--include-claude-dir` 時刪除 `{project}/.claude/` 整個目錄
   - 若指定 `--include-claude-dir`：會刪除整個 `.claude/`（包含其中的 `.claude/plans/`）
...
**輸出範例**:
```
Session: 012e2e2d-fde5-4146-bc11-15a436a8d46b
────────────────────────────────────────────
Project:     ~/Documents/Dev/FYP-LLM
Created:     2024-03-05 23:11
Tasks:       3 (1 in_progress, 2 completed)
Plans:       1
Todos:       0
Transcripts: ~/Library/Application Support/Claude/transcripts/...

Files to be deleted:
  - ~/.claude/tasks/012e2e2d-fde5-4146-bc11-15a436a8d46b/
  - ~/.claude/todos/*012e2e2d-fde5-4146-bc11-15a436a8d46b*
...
- 識別沒有任何關聯會話的專案 `.claude/` 目錄
- 選項: `--dry-run`, `--auto-remove` (自動刪除孤立資料)
- 其他旗標: `--verbose`, `--json`
...
├─────────────────────────────────────────────────────────────────┤
│  [Projects] [Sessions] [Cleanup]                    [Quit: Q]  │
├─────────────────────────────────────────────────────────────────┤
│  Projects (3)                                      [Delete: D] │
│  ─────────────────────────────────────────────────────────────  │
│  ▶ ✓ ~/Documents/Dev/FYP-LLM              3 sessions   2024-03 │
│    ✓ ~/Documents/Dev/Real-Alpha-Trader    1 session    2024-03  │
│    ✓ ~/Documents/Dev/quants               0 sessions   2024-03  │
│                                                                 │
│  Sessions in: ~/Documents/Dev/FYP-LLM                          │
│  ─────────────────────────────────────────────────────────────  │
│    ○ 012e2e2d-fde5-4146-bc11-...   tasks:3   2024-03-05  ●active│
│    ○ 2acfdebe-53d1-47de-8563-...   tasks:2   2024-03-05         │
│    ○ 51bd6114-6234-48b1-b03b-...   tasks:1   2024-03-05         │
├─────────────────────────────────────────────────────────────────┤
│  Selected: 1 project, 0 sessions                [Delete]       │
└─────────────────────────────────────────────────────────────────┘
```
...
│                                        │
│  Delete 1 project?                     │
│  ~/Documents/Dev/FYP-LLM               │
│                                        │
│  This will also delete 3 sessions:     │
│  • 012e2e2d-fde5-4146-bc11-...         │
│  • 2acfdebe-53d1-47de-8563-...         │
│  • 51bd6114-6234-48b1-b03b-...         │
│                                        │
...
def discover_sessions():
    ...
    # 2. 從 transcripts/ 目錄結構解析工作目錄
    #    路徑格式: -Users-kenc-Documents-Dev-FYP-LLM
    project_paths = parse_transcript_structure()
...
def delete_session(session_id):
    ...
    for pattern in direct_patterns:
        for file in glob(pattern):
            os.remove(file)  # 或 shutil.rmtree 用於目錄

    # paste-cache: 從 history.jsonl 提取 contentHash，檢查引用計數
    paste_hashes = extract_paste_hashes(session_id)  # 從 history 提取
    for content_hash in paste_hashes:
        ref_count = count_content_hash_references(content_hash)  # 掃描全部 history
        if ref_count == 1:  # 僅本 session 引用
            os.remove(f"~/.claude/paste-cache/{content_hash}.txt")
...
- Python 3.10+
- 標準庫: `pathlib`, `json`, `os`, `glob`, `argparse`
- 選用: `rich` (TUI 輸出), `questionary` (互動問答)
